import "./Experience.css";
export default function Experience() {
  return (
    <div className="experience">
      <h2 className="heading">Glentel: Junior Systems Administrator</h2>
      <h3 className="subheading">January 2023 - September 2024</h3>
      <p>
        <ul className="list">
          <li>
            Monitor and respond to multiple network and computer monitoring
            consoles to ensure optimal system performance.
          </li>
          <li>
            Efficiently manage projects with competing deadlines, ensuring
            timely delivery and prioritization of tasks to meet all objectives.
          </li>
          <li>
            Received and diagnosed service requests to resolve software and
            hardware related issues with a success rate of 99%.
          </li>
          <li>
            Collaborated with cross-functional teams across Canada, in process
            optimization of POS terminal configurations and driving monthly
            revenue of $100 million
          </li>
          <li>
            Perform various administrative duties, including generating reports,
            tracking invoices, and managing asset inventories.
          </li>
        </ul>
      </p>
    </div>
  );
}
